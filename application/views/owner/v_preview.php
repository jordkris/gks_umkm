<?php 
    $ci = get_instance();
    $status = 0;
    if($ci->uri->segment(3) == 'profile' || $ci->uri->segment(3) == 'cover') { 
      $status = 1;
    } else if(substr($ci->uri->segment(3),0,12) == 'product_logo') {
      $status = 2;
    } 
?>
<!DOCTYPE html>
              ...
              <div class="p-5">
                ...
                <div class="text-center">
                  ...
                  <small>Klik refresh jika terjadi error</small>
                   <hr />
                  <a href="<?php if($status == 1) { echo base_url('owner/profile'); } else if($status == 2) { echo base_url('owner/product/2');} ?>">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="crop" style="<?php if($upload == 'upload_product_logo') { echo 'display: none;'; } ?>"><i class="fas fa-save"></i> SIMPAN</button>
        <button type="button" class="btn btn-primary" id="crop2" style="<?php if($upload != 'upload_product_logo') { echo 'display: none;'; } ?>"><i class="fas fa-save"></i>  SIMPAN</button>
      </div>
    </div>
  </div>
</div>
<script>
  ...
  <?php if($upload != 'upload_product_logo') { ?>
  $("#crop").click(function(){
          ...
          var reader = new FileReader();
           ...
           reader.onloadend = function() {
              ...
              $.ajax({
                  type: "POST",
                  dataType: "json",
                  url: "<?= base_url('owner/'.$upload.'/'.$profile['id']); ?>",
          ...
          var reader = new FileReader();
           ...
           reader.onloadend = function() {
              ...
              $.ajax({
                  type: "POST",
                  dataType: "json",
                  url: "<?= base_url('owner/'.$upload.'/'.$product_id); ?>",
                  data: {image: base64data},
                  success: function(data){
                      console.log(data);
                  }
  ...
  function redir() {
    <?php 
    if($status == 1) { ?>
      window.open('<?= base_url('owner/profile'); ?>','_self');
    <?php } else if($status == 2) { ?>
      window.open('<?= base_url('owner/product/2'); ?>','_self');
    <?php } ?>
  }
