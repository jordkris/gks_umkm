<!-- Begin #carousel-section -->
    ...
    <hr />
    <h5 class="services-header-title text-center"><a href="<?= base_url('product'); ?>">Semua Produk</a> <i class="fas fa-chevron-right"></i> Profil <?= $subtitle; ?></h5>
    <!-- Begin #services-section -->
    <section id="services" class="services-section section-global-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-4 bg_general">
                    <div class="row">
                        <div class="services-header">
                            <h3 class="services-header-title">Profil <?= $subtitle; ?></h3>
                            <p class="services-header-body"><hr>
                        </div>
                    </div>
                    <div class="row">
                        <?php foreach($users as $u) { if($u['id'] == $id_owner) { ?>
                        <div class="col-md-12" style="margin-bottom: 81px">
                            <!-- height 530 standart -->
                            <iframe src="<?= base_url('product/show_profile/'.$u['id']);?>" style="max-height: 800px;overflow: hidden;" width="100%" height="600" scrolling="no" frameBorder="0"></iframe>
                        </div>
                        <?php } } ?>
                    </div>
                </div>
                <div class="col-md-8 bg_general">
                    <div class="row">
                        <div class="services-header">
                            <h3 class="services-header-title">Semua Produk Usaha <?= $subtitle; ?></h3>
                            <p class="services-header-body"><hr>
                        </div>
                    </div>
                    <div class="row">
                        <?php foreach($product as $p) { if($p['id_user'] == $id_owner) { ?>
                        <div class="col-md-6">
                            <!-- height 530 standart -->
                            <iframe src="<?= base_url('product/show_product/'.$p['id']);?>" style="max-height: 800px;overflow: hidden;" width="100%" height="680" scrolling="no" frameBorder="0"></iframe>
                        </div>
                        <?php } } ?>
                    </div>
                </div>
